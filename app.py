...
def create_dashboard(results, result_df, natural_df, human_df):
    ...
    with col1:
        st.metric("🌍 分析国家数", len(result_df), delta=None)
    with col2:
    ...
    with tab1:
        ...
        with col_left:
            ...
            display_df.columns = ['国家', '协调度D', '等级', '地理得分', '经济得分']
        ...
        top_n = st.slider("显示前N个国家&地区", 5, 30, 15)

        top_countries = result_df.head(top_n)
        colors = ['#2ecc71' if d >= 0.6 else '#f39c12' if d >= 0.4 else '#e74c3c'
    ...
    with tab2:
        ...
        with col2:
            color_by = st.radio("颜色标记", ["协调等级", "地理得分", "经济得分"], horizontal=True)
    ...
    with tab4:
        ...
        # 选择国家
        selected_country = st.selectbox("选择国家&地区查看详情", result_df['country'].values)
        ...
        with col_d3:
            ...
            st.metric("📈 排名", f"第 {result_df[result_df['country'] == selected_country].index[0] + 1} 名")
        ...
        fig_compare_country.update_layout(
            title=f"{selected_country} vs 全部国家平均值",
            yaxis_title="数值",
            barmode='group'
        )
...
def main():
    ...
    with st.sidebar:
        ...
        # 模型参数
        st.subheader("🎛️ 模型参数")
        alpha = st.slider("地理环境系统权重 α", 0.0, 1.0, 0.5, 0.1)
        beta = st.slider("社会经济系统权重 β", 0.0, 1.0, 0.5, 0.1)
        ...
        with st.spinner("正在加载数据并运行分析..."):
            try:
                ...
                result_df = pd.DataFrame({
                    ...
                    'level': [l[0] for l in levels]
                }).sort_values('coordination_D', ascending=False).reset_index(drop=True)

                # 显示成功消息
                st.success(f"✅ 分析完成！共分析 {len(result_df)} 个国家")
                ...
                with col_d2:
                    ...
                    地理系统权重:
                    {dict(zip(results['geo_columns'], results['geo_weights']))}

                    经济系统权重:
                    {dict(zip(results['socio_columns'], results['socio_weights']))}
                    """
                    st.download_button(
                        label="📄 下载分析报告 (TXT)",
                        data=report,
